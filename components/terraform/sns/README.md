...
| `sns_topic_policy_json` ("") | a full policy that replaces the generated one |
...
- **Key policy.** A service publishing to the topic encrypts with the key as a
  ...
  kms/main turns on `allow_eventbridge` in every stack, which covers
  EventBridge rules targeting the topic. Other publishers (CloudWatch alarms,
  S3 notifications) need their own key-policy statement.
- **SQS subscribers.** SNS delivers to a queue encrypted with kms/main through
  kms/main's `allow_sns` statement (on in every stack). The queue itself must
