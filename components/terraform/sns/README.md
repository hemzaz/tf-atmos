...
| `sns_topic_policy_json` ("") | extra policy JSON merged into the generated one as a source document; the generated statements (the TLS deny included) win on a `Sid` clash, and an Allow for principal `*` without a `Condition` is rejected |
