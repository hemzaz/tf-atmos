...
| `sns_topic_policy_json` ("") | extra policy JSON merged into the generated one as a source document; the generated statements (the TLS deny included) win on a `Sid` clash. Its statements are used as written and are **not** rescoped to this topic, so set each `Resource` to the topic ARN yourself. Allow statements are rejected if they use `NotPrincipal`, a principal with a wildcard inside it (`arn:aws:iam::*:root`), or principal `*` without a `Condition` that pins the caller: one of `aws:SourceAccount`, `aws:SourceArn`, `aws:SourceOwner`, `aws:SourceOrgID`, `aws:PrincipalOrgID`, `aws:PrincipalAccount` or `aws:PrincipalArn` (any case), under a positive operator (not `...Not...`, `...IfExists` or `Null`, which let a caller without the key through) and with no value that is just `*`. Element names (`Statement`, `Effect`, `Principal`, ...) and the `Effect` value are read case-insensitively, as the policy parser reads them |
