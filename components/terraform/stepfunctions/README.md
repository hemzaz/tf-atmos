...
| `logging_configuration` (`{}` → `level = "OFF"`, `include_execution_data = false`) | `level`: `ALL`, `ERROR`, `FATAL` or `OFF`. The log group is always created (`log_group_name` is never null); a non-`OFF` level attaches the execution role's log-delivery permissions and points the state machine at it. `OFF` just stops the state machine writing to it |
| `tracing_enabled` (`false`) | Enables AWS X-Ray tracing and attaches the execution role's X-Ray write permissions this requires |
