...
output "webhook_url" {
  ...
  value       = one(aws_codebuild_webhook.this[*].payload_url)
}

output "webhook_secret" {
  description = "Secret token for the webhook"
  value       = one(aws_codebuild_webhook.this[*].secret)
  sensitive   = true
}
...
output "project_url" {
  ...
  value       = "https://${data.aws_region.current.region}.console.aws.amazon.com/codesuite/codebuild/projects/${aws_codebuild_project.this.name}"
}
