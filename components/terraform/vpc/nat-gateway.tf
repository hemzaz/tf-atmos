locals {
  ...
  nat_gateway_subnet_indices = var.nat_gateway_strategy == "one_per_az" ? 
    [for i in range(local.nat_gateway_count) : i] : 
    (var.nat_gateway_strategy == "single" ? [0] : [])
}
...
data "aws_availability_zone" "available" {
  ...
  name  = var.nat_gateway_azs != null && length(var.nat_gateway_azs) > count.index ? 
          var.nat_gateway_azs[count.index] : 
