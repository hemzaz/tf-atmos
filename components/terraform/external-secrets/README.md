...
|---|---|---|
| region, cluster_name, host, cluster_ca_certificate, oidc_provider_arn, oidc_provider_url, tags (must have a non-empty `Environment` value), kms_key_arn (required only when `enabled` is true; null/empty is accepted on a disabled instance) | chart_version, create_default_cluster_secret_store, create_certificate_secret_store, namespace/service_account_name, secret_path_prefixes, ssm_parameter_path_prefixes, secret_path_context_prefixes, rds_managed_secret_access | external_secrets_role_arn/name, default_cluster_secret_store_name (consumed via `!terraform.state`, e.g. by `eks-backend-services`), policy_arn/name |
