...
- This component does not provision the AWS Auto Scaling service-linked
  role. `kms/main`'s `allow_autoscaling_ebs` grant (see `../kms/README.md`)
  targets the account root with an `aws:PrincipalArn` condition instead of
  naming the role directly, so it never depends on the role existing at
  apply time and this component has no edge to `kms/main`.
