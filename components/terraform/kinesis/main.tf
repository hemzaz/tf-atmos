...
# calling PutRecord[s], ... each has its own), so it exposes two ready-to-use
# IAM policy documents instead:
#   - reader_policy: the Kinesis actions a consumer needs to read the stream
#     (GetRecords/GetShardIterator/DescribeStream[Summary]/ListShards/
#     ListStreams, plus SubscribeToShard/DescribeStreamConsumer on any
#     enhanced fan-out consumer ARNs), plus kms:Decrypt scoped to this
#     stream's own encryption context - the same Resource/Condition pair
#     AWS's own docs show for granting a Kinesis reader (e.g. Firehose
#     reading an encrypted source stream) kms:Decrypt.
...
#     producer needs to generate a data key to encrypt each record).
...
# output, and this stream's writer_policy folds its Statement entries in.
...
locals {
  ...
  # (a full {Version, Statement} document), merged into this stream's own
  # writer_policy below. See additional_policy_json's description.
  additional_statements = var.additional_policy_json != null ? jsondecode(var.additional_policy_json).Statement : []
