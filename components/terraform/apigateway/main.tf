...
resource "aws_api_gateway_method" "method" {
  ...
  dynamic "request_parameters" {
    for_each = length(var.api_methods[count.index].request_parameters) > 0 ? [1] : []
    content {
      for_each = var.api_methods[count.index].request_parameters
      content {
        "${key}" = value
      }
    }
...
resource "aws_api_gateway_integration" "integration" {
  ...
  dynamic "request_parameters" {
    for_each = length(var.api_integrations[count.index].request_parameters) > 0 ? [1] : []
    content {
      for_each = var.api_integrations[count.index].request_parameters
      content {
        "${key}" = value
      }
    }
  ...
  dynamic "request_templates" {
    for_each = length(var.api_integrations[count.index].request_templates) > 0 ? [1] : []
    content {
      for_each = var.api_integrations[count.index].request_templates
      content {
        "${key}" = value
      }
    }
